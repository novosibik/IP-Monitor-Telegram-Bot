...
### 🧪 Проверка окружения на Ubuntu 25.10

Минимальные проверки, которые можно выполнить до старта бота:

1. Убедитесь, что системные зависимости ставятся без ошибок:
   ```bash
   sudo apt-get update
   ...
   ```
2. Соберите байткод для обоих скриптов (проверка синтаксиса без Telegram-токена):
   ```bash
   python3 -m compileall monitor.py report.py
   ```
3. Прогоните одиночную проверку пинга (не требует реального токена, т.к. Telegram-поллинг не запускается):
   ```bash
   python3 monitor.py --once
   ```
   В результате создадутся `ip_monitor_log.csv` и `ip_monitor.prom` — так можно убедиться, что зависимости установлены и пинг работает.
